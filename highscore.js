...
function getHighScoreTable() {
    ...
    for (var i = 0; i < 5; i++) {
		...
		table.push(new ScoreRecord(info[0], info[1]));
    }
...
function setHighScoreTable(table) {
    for (var i = 0; i < 5; i++) {
		...
		var infoStore = table[i].name + "~" + table[i].score;
		setCookie(cookie_name, infoStore);
    }
}
...
function addHighScore(record, node) {
    ...
    if (record.highlight) name.setAttribute("fill", "blue");	
    ...
    if (record.highlight) score.setAttribute("fill", "blue");    
